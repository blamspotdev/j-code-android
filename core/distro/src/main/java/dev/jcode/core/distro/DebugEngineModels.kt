...
object DebugEngineCatalog {
    val BUILT_IN: List<DebugEngineEntry> = listOf(
        DebugEngineEntry(
            ...
            installCommand = "python3 -m pip install --break-system-packages -U debugpy",
            verifyCommand = "python3 -c 'import debugpy, sys; print(debugpy.__version__)'",
            uninstallCommand = "python3 -m pip uninstall -y --break-system-packages debugpy",
            adapterCommand = "python3 -m debugpy.adapter",
            transport = "stdio",
            debugType = "python",
            updateCheckCommand = "test -n \"\$(python3 -m pip list --outdated 2>/dev/null | grep -i '^debugpy ')\"",
