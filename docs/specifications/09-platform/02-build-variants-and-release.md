...
is also `dev.blamspot.jcode`, and the Kotlin, Java, AIDL and JNI sources sit under it: at 1.7.1 the
...
editing a string — both were done for 1.7.1 rather than deferred:
...
- **The virtual device's guest apps** are now `dev.blamspot.jcode.vdevice.*`. That string *is* the
  ...
  `launcher`, was added at 1.7.1 and was born in the new package — the device's home screen became an
...
- **`tools/java-dap`** is now `dev.blamspot.jcode.javadap`. The jar is not built during a JCode
  ...
  launches `dev.blamspot.jcode.javadap.Main` — **that release must exist before 1.7.1 ships**, or
...
```kotlin
val jcodeVersion = "1.7.1"                                   // the train being prepared
...
train; merges do not move it. Previews of that train are built and published as `1.7.1-beta.N`, and
`1.7.1` is published from the same line of commits when it is ready — publishing it is what opens
the next train.

```
main = 1.7.1        merge, merge, merge          (jcodeVersion unchanged)
                    publish v1.7.1-beta.1        pre-release, dev.blamspot.jcode.beta
                    merge
                    publish v1.7.1-beta.2        pre-release
                    publish v1.7.1               release, dev.blamspot.jcode
...
|---|---|---|
| `1.7.1-alpha.1` | 1060101 | `N` |
| `1.7.1-beta.1` | 1060131 | `30 + N` |
| `1.7.1-beta.2` | 1060132 | |
| `1.7.1-rc.1` | 1060161 | `60 + N` |
| `1.7.1` | 1060199 | `99` |

The old derivation ignored the suffix entirely, so `1.7.1-beta.1`, `1.7.1-beta.2` and `1.7.1` all
...
A new train starts back at `beta.1` on its own, because `v1.7.1-beta.*` is a different prefix.
...
`1.7.1-beta.2 < 1.7.1-rc.1 < 1.7.1` — covered by `app/src/test/java/dev/blamspot/jcode/UpdateCheckerTest.kt`.
...
**The release this was built for.** 1.7.1 is the release that changes the package, from `dev.jcode`
to `dev.blamspot.jcode`. 1.6.1 is the one that makes it survivable: it shipped the export side
above, so every 1.6.1 install hands its environment over on its own when it updates into 1.7.1.
...
|---|---|---|---|---|
| `beta`, label blank | `1.7.1-beta.N` (next) | `v1.7.1-beta.N` | `dev.blamspot.jcode.beta` | pre-release |
| `stable` | `1.7.1` | `v1.7.1` | `dev.blamspot.jcode` | release |
