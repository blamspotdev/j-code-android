...
| **Every member of `android.app.PropertyInvalidatedCache`**, and `ApplicationPackageManager.mHasSystemFeatureCache` with it | Denied. The class answers `getDeclaredMethods()` and `getDeclaredFields()` with **nothing at all**; the field is `blocked, reflection, denied`. That is the cache in front of `hasSystemFeature`, so what hardware a guest believes the device has is frozen for the life of the process | Nothing to get in front of — the question is asked on `PackageManager`, which is an abstract class with a couple of hundred members and cannot be subclassed by hand the way `Context` could. The device **restarts** instead when the bench changes what hardware it has: `VirtualDevicePolicy.setMode` → `AppSandbox.restartForHardware`. See [App sandbox architecture §7m](01-app-sandbox-architecture.md) |
