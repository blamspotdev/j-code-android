 ...
 *
 * A Beta build is not the release app with a label on it — it is `dev.jcode.beta`, its own
 * `applicationId`, installed alongside `dev.jcode` with its own data. So "is there an update?" is
